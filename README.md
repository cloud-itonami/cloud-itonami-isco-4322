...
- `src/production_clerking/store.kotoba` — `Store` protocol +
  `MemStore`: registered production orders, committed records, an
  append-only audit ledger.
- `src/production_clerking/advisor.kotoba` — `Advisor` protocol;
...
- `src/production_clerking/governor.kotoba` —
...
- `src/production_clerking/actor.kotoba` — `build-graph`, `run-request!`,
