(ns production-clerking.actor
  "ProductionClerkingActor — the ISCO-08 4322 independent production-
  clerking actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  tracking operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the ProductionAdvisor can never
  directly commit a record or dispatch a robot action the
  ProductionClerkingGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [production-clerking.advisor :as advisor]
            [production-clerking.governor :as governor]
            [production-clerking.store :as store]))

(defn build-graph
  "Build a compiled ProductionClerkingActor graph. `store` implements
  `production-clerking.store/Store`. `advisor` implements
  `production-clerking.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:order-id (:order-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
