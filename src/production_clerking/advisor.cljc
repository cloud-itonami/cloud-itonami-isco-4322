(ns production-clerking.advisor
  "ProductionAdvisor — proposes a production-clerking operation
  (track a job ticket, reconcile a count, close a production run with
  an unresolved discrepancy, override a scheduling conflict affecting
  safety staffing) for a registered order. The advisor is swappable:
  ...
  and has no notion of order provenance or discrepancy-closure/
  staffing-override risk; `production-clerking.governor` is the
  independent system that decides whether the proposal may proceed,
  per the itonami actor pattern.

  A proposal is a map:
    {:op :track|:reconcile|:close-run-with-discrepancy|:override-scheduling-conflict
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for order " (:order-id request))})
...
(def ^:private system-prompt
  "You are a production-clerking advisor. Given a tracking operation
